...
function heat-cfntools {
    ...
    if [ "$ID" == "ubuntu" ] && [ "$VERSION_ID" == "14.04" ]; then
        # Package is broken on Trusty. (LP #1398248) Use the utopic version.
        wget http://mirror.internode.on.net/pub/ubuntu/ubuntu/pool/universe/h/heat-cfntools/heat-cfntools_1.2.7-2_all.deb -O /tmp/heat-cfntools_1.2.7-2_all.deb
...
function os-config {
    ...
    if [ "$ID" == "ubuntu" ] && [ "$VERSION_ID" == "14.04" ]; then
        ...
        apt-get -qq --no-install-recommends -y install python-anyjson \
            ...
            python-six
        pushd /tmp
        for package in python-os-apply-config_0.1.14-1_all.deb \
            python-os-refresh-config_0.1.2-1_all.deb \
            python-os-collect-config_0.1.15-1_all.deb ; do

            wget http://mirror.internode.on.net/pub/ubuntu/ubuntu/pool/universe/p/python-os-collect-config/${package}
            dpkg -i ${package}
            rm -f ${package}
        done
        popd
